...
var bookFlip = {
  _width: [],
  //flipbook pages width
  _height: [],
  //flipbook pages height
  active: false,
  //flipbook mode on
  _spreadBk: NaN,
  //spread mode backup to restore
  _evSpread: null,
  //spread mode changed default event handler 
  _spread: NaN,
  //spread page mode
  toStart: false,
  //PDFjs require flipbook at start
  _intoView: null,
  //link handler default function
  _visPages: null,
  //visible pages function
  _ready: false,
  //ready to start flipbook
  // event listeners when bookFlip need different handling 
  init: function init() {
    var _this = this;

    $(document).on('rotationchanging', function () {
      _this.rotate();
    });
    $(document).on('scalechanging', function () {
      _this.resize();
    });
    $(document).on('pagechanging', function () {
      _this.flip();
    });
    $(document).on('documentinit', function () {
      _this.stop();
    ...
    $(document).on('scrollmodechanged', function () {
      ...
      if (scroll === 3) _this.start();else _this.stop();
      var button = PDFViewerApplication.appConfig.secondaryToolbar.bookFlipButton;
      button.classList.toggle('toggled', scroll === 3);
    });
    $(document).on('switchspreadmode', function (evt) {
      _this.spread(evt.originalEvent.detail.mode);
    ...
    $(document).on('baseviewerinit', function () {
      PDFViewerApplicationOptions.set('scrollModeOnLoad', 3);
      _this._intoView = PDFViewerApplication.pdfViewer.scrollPageIntoView;
      _this._visPages = PDFViewerApplication.pdfViewer._getVisiblePages;
    });
  },
  // startup flipbook
  start: function start() {
    ...
    viewer.scrollPageIntoView = function (data) {
      return _this2.link(data);
    };

    viewer._getVisiblePages = function () {
      return _this2.load();
    };
    ...
    $('#viewer').turn({
      ...
      when: {
        turned: function turned(event, page) {
  ...
  rotate: function rotate() {
    if (!this.active) return;
    var _ref = [this._width, this._height];
    this._height = _ref[0];
    this._width = _ref[1];
    this.resize();
  },
  // change flipbook spread mode
  spread: function spread(spreadMode) {
    if (!this.active) return;
  ...
  _spreadType: function _spreadType() {
    return this._spread === 0 ? 'single' : 'double';
  },
  _spreadMult: function _spreadMult() {
    return this._spread === 0 ? 1 : 2;
  },
  _size: function _size(page, request) {
    var size;
